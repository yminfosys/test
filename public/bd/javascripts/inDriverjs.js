...
function veryfyMobileNumber(){
   ...
   var mobile=$("#mobile").val()
    if(mobile.length===10){        ;
        ...
        $.post('/bd/drv/checkMobileExist',{mobile:mobile},function(data){
            if(data=='exist'){
                ///////Login Function////
...
function loginprocess(){
    ...
    var mobile=$("#mobile").val();
    $.post('/bd/drv/login',{password:password,mobile:mobile},function(data){
        if(data=='success'){
            window.location='/bd/drv'
        }else{
 ...
 ///////Handel Socket io  parameter/////// 
        ...
        setTimeout(function(){
            ...
            $.post('/bd/drv/resetRingtone',{pilotID:data.pilotID},function(dat){
                console.log(dat);
            });
        },14*1000)
        }       
        ...
        socket.on('CallAcceptListDisplay', function (resp) {
            ...
            if(resp.pilotID==getCookie("pilotID")){
                $.post('/bd/drv/clineDetalls',{pilotID:resp.pilotID,CustID:resp.CustID,bookingID:resp.bookingID},function(data){
    ...
    if(data.pilotID==getCookie("pilotID")){
        ...
        window.location='/bd/drv'
         ...
         function acceptRide(){                        
            ...
            $.post('/bd/AcceptCallByDriver',{pilotID:pilotID,CustID:CustID},function(data){            
            if(data){ 
                $("#ringtone").css({"display":"none"});
  ...
  ((navigator.platform.indexOf("iPhone") != -1) || 
   ...
   (navigator.platform.indexOf("iPod") != -1)){
     ...
     function clineLocated(){ 
            $.post('/bd/drv/clinelocated',{CustID:$("#CustID").val()},function(respon){
     ...
     function otpinput(){        
        ...
        if(valu.length >3){
            ...
            if($("#OrderOTP").val()==valu){
                $.post('/bd/drv/startRide',{CustID:$("#CustID").val(),bookingID: $("#orderNO").text()},function(data){
...
$.post('/bd/drv/cancelOrderByDriver',{bookingID:bookingID},function(data){
    console.log(data);
    if(data=="ok"){
        $("#pickDrop-Content").css({"display":"none"});
        $("#OTP-Content").css({"display":"none"});
