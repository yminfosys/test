...
function veryfyMobileNumber(){
   ...
   var mobile=$("#mobile").val()
    if(mobile.length===10){        ;
        ...
        $.post('/uk/drv/checkMobileExist',{mobile:mobile},function(data){
            if(data=='exist'){
                ///////Login Function////
                ...
                //////Send OTP////
                $.post('/uk/drv/otpSend',{mobile:mobile,otp:otp},function(data){
...
function loginprocess(){
    ...
    var mobile=$("#mobile").val();
    $.post('/uk/drv/login',{password:password,mobile:mobile},function(data){
        if(data=='success'){
            window.location='/uk/drv'
        }else{
 ...
 ///////Handel Socket io  parameter/////// 
        ...
        setTimeout(function(){
            ...
            $.post('/uk/drv/resetRingtone',{pilotID:data.pilotID},function(dat){
                console.log(dat);
            });
        },14*1000)
        }       
        ...
        socket.on('CallAcceptListDisplay', function (resp) {
            ...
            if(resp.pilotID==getCookie("pilotID")){
                $.post('/uk/drv/clineDetalls',{pilotID:resp.pilotID,CustID:resp.CustID,bookingID:resp.bookingID},function(data){
    ...
    if(data.pilotID==getCookie("pilotID")){
        ...
        window.location='/uk/drv'
         ...
         function acceptRide(){                        
            ...
            $.post('/uk/AcceptCallByDriver',{pilotID:pilotID,CustID:CustID},function(data){            
            if(data){ 
                $("#ringtone").css({"display":"none"});
  ...
  ((navigator.platform.indexOf("iPhone") != -1) || 
   ...
   (navigator.platform.indexOf("iPod") != -1)){
     ...
     function clineLocated(){ 
            $.post('/uk/drv/clinelocated',{CustID:$("#CustID").val()},function(respon){
     ...
     function otpinput(){        
        ...
        if(valu.length >3){
            ...
            if($("#OrderOTP").val()==valu){
                $.post('/uk/drv/startRide',{CustID:$("#CustID").val(),bookingID: $("#orderNO").text()},function(data){
...
$.post('/uk/drv/cancelOrderByDriver',{bookingID:bookingID},function(data){
    console.log(data);
    if(data=="ok"){
        $("#pickDrop-Content").css({"display":"none"});
        $("#OTP-Content").css({"display":"none"});
