  ...
  function circleMarker(position){
    ...
    if(!centerMarker){
       ...
       // icon:'http://www.robotwoods.com/dev/misc/bluecircle.png',
        icon:new google.maps.MarkerImage('/uk/images/bluecircle.png',
...
var dropMarker=new google.maps.Marker({        
       // icon:'http://www.robotwoods.com/dev/misc/bluecircle.png',
        icon:new google.maps.MarkerImage('/uk/images/drop.png',
                                        new google.maps.Size(50,50),
                                        new google.maps.Point(0,0),
...
// icon:'http://www.robotwoods.com/dev/misc/bluecircle.png',
    icon:new google.maps.MarkerImage('/uk/images/pickup.png',
                                    new google.maps.Size(50,50),
                                    new google.maps.Point(0,0),
                                    new google.maps.Point(25,50)),
    map:map
...
content: "Drop"
  ...
  function findPlaceBylntlng(latlng){
    var searchmod=$("#ModeofSearch").val();
     $.post('/uk/geoplace',{lat:latlng.lat,lng:latlng.lng},function(data){       
    if(searchmod=='0'){
       ...
       ///////Update Demand Location////////
        $.post('/uk/updateDemndLocation',{lat:latlng.lat,lng:latlng.lng},function(dd){
          console.log(dd);
        })
  ...
  ///////Nearest Driver Marker////////
    ...
    driverLocetion.forEach(function(val,indx){
      ...
      driverMarkers.push(new google.maps.Marker({
        ...
        icon:{
            url: "/uk/images/DriverMarker"+travelmod+".png", // url
    ...
    document.getElementById("naxtBtn").addEventListener("click", function(){
      ...
      var countt=0;
          $.each($(".modeImg"),function(i){
            j=i+1;
            $.post('/uk/getDistance',{travelmod:j,orig:''+Number(origin.lat)+' , '+Number(origin.lng)+'',diste:''+Number(dist.lat)+' , '+Number(dist.lng)+''},function(data){
            ...
            $.post('/uk/getPrice',{travelmod:data.travelmod,distance:distance},function(dataa){
              ...
              if(countt==$(".modeImg").length-1){
                ...
                //$("#booimg").html('<img class="tm-img" src="/uk/images/tm'+travelmod+'.png">');
                ...
                if(Number(tmPreRidePrice)>0){
                  ...
                  $("#promoMsg").html('<marquee>Reduce price @ &#163;'+tmPreRidePrice+'/Mile Tab on Pre-Ride button</marquee>')
...
//////// End Contunue Button////////////   
      ...
      document.getElementById("placeList").addEventListener("click", function(e) {
        ...
        // alert("clicked " + e.target.innerText);
         //alert("Placeid " + e.target.querySelector('input').value);
          ...
          $.post('/uk/placeidtogeocod',{placeid:e.target.querySelector('input').value},function(data){
            if(data.status=='OK'){
 ...
 function activTravalModeNerestTime(data,status){
      ...
      var origin=JSON.parse(getCookie("pickuplatlong")) ;
      $.post('/uk/distbtwnActive',{orig:''+Number(origin.lat)+' , '+Number(origin.lng)+'',diste:''+Number(data[1])+' , '+Number(data[0])+''},function(data){
...
$("#promoMsg").appendTo(map.getDiv());
  ...
  function cityLocation(position){
    ...
    if(getLocation==0){
      getLocation=1;
      //alert("gg");
      $.post('/uk/preRideUpdateCitywisePrice',{lat:position.coords.latitude, lng:position.coords.longitude},function(data){
        if(data){
          console.log(data)
