...
function initMap() {  
    ...
    directionsDisplay.setMap(map);
     ...
     function getDriverposition(reqs){
            ...
            $.post('/uk/getDriverposition',{pilotID:reqs.pilotID},function(resp){
                console.log(resp)
    ...
    //        timerR=setInterval(function(){
    //        $.post('/uk/getFinishBooking',{bookingID:reqs.bookingID},function(resp){
...
///////////Initiate page Parametter///////
        function pageInit(){         
         ...
         $.post('/uk/rideDriverBookingDetails',{bookingID:bookingID},function(data){
         ...
         if($("#orderStage").val()=='accept'){
            ...
            if($("#orderStage").val()=='startRide'){
                ...
                if($("#orderStage").val()=='finishRide'){
                    ...
                    $.post('/uk/getFinishBooking',{bookingID:data.ride.bookingID},function(resp){
                        console.log(resp.totalamount);
                        $("#billAndfeedback").css({"display":"block"});
            ...
            $("#footer").html('<div id="driver-content">\
             ...
             <div class="col-xs-10 col-xs-offset-1 col-sm-10 col-sm-offset-1">\
                <img class="drive-img img-circle" src="/uk/'+data.driver.photo+'">\
  ...
  document.getElementById("continueNextRide").addEventListener("click", function(){    
    $.post('/uk/setAllNormalandFinished',{},function(data){
      if(data){       
...
function cancelOrderByCust(bookingID){
  ...
  if (r == true){ 
    var cost=$("#cancelCharge").val();
      ...
      $.post('/uk/CancelRideByCust',{bookingID:bookingID,cancelCost:cost},function(data){
        if(data){
            setCookie("cancelTimer","",1);
            window.location='/uk'
        }
