 ...
 //////End Circle Marker//////
    ...
    ////////Map Ctrl Marker//////
        var demand=0;
         ...
         $.post('/uk/drv/getDemadndArea',{lat:map.getCenter().toJSON().lat,lng:map.getCenter().toJSON().lng},function(data){
          console.log(data)
    ...
    DemandLocetion.forEach(function(val,indx){
      ...
      DemandMarkers.push(new google.maps.Marker({
        ...
        icon:{
            url: "/uk/images/demand-marker.png", // url
...
///////Clear Demand Marker////////
  ...
  if(setSystem=="ONLINE"){
    ...
    if(document.getElementById("toggle").checked == true){      
      ...
      ////////////////      
      setTimeout(function(){
        $.post('/uk/drv/dutyUpdate',{duty:'offline'},function(data){
          console.log(data);
      ...
      if(getCookie("openAcceptWindow")){
        ...
        var tt=setInterval(function(){
          if(!getCookie("openAcceptWindow")){
            ...
            $.post('/uk/drv/resetRingtone',{pilotID:inData.pilotID},function(dat){
              console.log(dat);
            })
            clearInterval(tt);
          }
  ...
  document.getElementById("continueNextRide").addEventListener("click", function(){    
    $.post('/uk/drv/finishEverythingAndSetNormal',{
      CustID:$("#CustID").val(),
  ...
  function incetiveAndBooking(){
    $.post('/uk/drv/bookingIncentiveDetails',{},function(data){
  ...
  function pageInitiate(){
    ...
    if($("#bookingID").val()){
      $.post('/uk/drv/getPageInitiateDetails',{bookingID:$("#bookingID").val()},function(data){       
