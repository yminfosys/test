   ...
   var mobile=$("#mobile").val()
    if(mobile.length===10){        ;
       ...
       // alert(otp)
        ...
        $.post('/uk/preDrv/checkMobileExist',{mobile:mobile},function(data){
            if(data=='exist'){
                ///////Login Function////
...
function loginprocess(){
    ...
    var mobile=$("#mobile").val();
    $.post('/uk/preDrv/login',{password:password,mobile:mobile},function(data){
        if(data=='success'){
            window.location='/uk/Predrv'
        }else{
 ...
 function logout(){
    if(getCookie("setSystem")){
     alert("1st Go OffLine and Try Again")
        ...
        window.location="../uk/preDrv/logout"
    }
 }
 ...
 /////////Driver Page ////////////
  ...
  if(data.pilotID==getCookie("pilotID")){
      ...
      ////Check existing call////
      $.post('/uk/existingPrerideCall',{pilotID:data.pilotID,driverBusy:data.driverBusy},function(rides){
  ...
  function googlemapbtn(a,b,bookingID){
    ...
    if(a==1){
        ...
        ((navigator.platform.indexOf("iPhone") != -1) || 
         ...
         $.post("/uk/startCarLoogbook",{bookingID:bookingID},function(data){console.log(data)});
    }else{
        if(a==2){
       // alert("drop")
        if /* if we're on iOS, open in Apple Maps */
...
function clineLocated(indx){
    ...
    $.post('/uk/preRideClinelocated',{CustID:CustID,bookingID:bookingID},function(respon){
...
function otpinput(otp,indx){         
   ...
   if(valu.length >3){       
       if(otp==valu){
           $.post('/uk/preRideStartRide',{CustID:CustID,bookingID:bookingID},function(responce){
                if(responce){
                    $("#OTP-Content").css({"display":"none"});
...
function cancelOrderByDriver(bookingID,indx){
    $.post('/uk/cancelOrderByPreDriver',{bookingID:bookingID},function(data){
        console.log(data);
        if(data=="ok"){
            $("#listItem"+indx+"").css({"display":"none"})
            $("#OrderCancelTimer"+indx+"").css({"display":"none"});
